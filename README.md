...
- 轻松管理用户的输入内容
  1. 清晰结构化的表单逻辑定义
  1. 包含过滤、异步验证、及错误提示
  1. 每条数据可绑定多条校验规则，每条规则都可对应错误提示，或动态的错误提示
  1. 可轻易实现表单数据之间的约束
  1. 提供 api 方法，可调用 api 方法，控制表单数据，以便配合使用其他独立封装的 UI 控件
- 无需操作 dom，数据与视图绑定
- 不依赖 UI 库
- 适用于任何浏览器（[前提条件](#兼容情况)）
...
**定义表单的数据**, key 与视图的 name 对应，如有两个数据`foo`、`bar`：
...
假如有些输入控件不是原生的元素，无法通过 formController 自动数据绑定，则可调用`fc.updData(name, value)`来绑定数据。

例子：还是引用以上定义的一个数据`foo`，但是 html 上没有该输入框元素，而是一个另外封装的 UI 控件`InputUI`。假如该 UI 如下声明：
...
3. 错误提示的容器的特征需要为`span.err`，及一些`data-`的定义。
...
import {
  ...
  FormSmsCode,
  FormImgCode
} from 'form-controller/dist/components/react/index.jsx';
